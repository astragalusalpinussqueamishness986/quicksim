<template>
  <div v-if="data" class="trend-comparison rounded-2xl border border-white/10 bg-white/[0.03] p-5 space-y-4">
    <div class="flex items-center gap-2 text-sm text-slate-400">
      <span>📊</span>
      <span class="font-medium">搜索热度对比</span>
      <span class="text-xs text-slate-600">（基于搜索引擎真实数据）</span>
    </div>
    ...
    <div>
      ...
      <div class="h-3 rounded-full overflow-hidden bg-white/5 flex">
        <div
      ...
      <div class="text-center text-xs text-slate-500 mt-1">{{ data.comparison.ratio }}</div>
    </div>

    <!-- Related searches -->
    <div class="grid grid-cols-2 gap-4">
      <div>
        <p class="text-xs text-slate-500 mb-2">相关搜索</p>
        <div class="flex flex-wrap gap-1">
          ...
          >{{ s }}</span>
          <span v-if="!data.choiceA.suggestions.length" class="text-xs text-slate-600">暂无数据</span>
        </div>
      </div>
      <div>
        <p class="text-xs text-slate-500 mb-2">相关搜索</p>
        <div class="flex flex-wrap gap-1">
          ...
          >{{ s }}</span>
          <span v-if="!data.choiceB.suggestions.length" class="text-xs text-slate-600">暂无数据</span>
        </div>
      </div>
    </div>
  ...
  <!-- Loading state -->
  <div v-else-if="loading" class="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center">
