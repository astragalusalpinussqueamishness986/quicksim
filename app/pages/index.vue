  ...
  <div>
    ...
    <section class="text-center pt-8 pb-6">
      ...
      <div class="flex justify-end mb-2">
        <NuxtLink to="/profile" class="flex items-center gap-1.5 text-sm text-slate-400 hover:text-white transition-colors px-3 py-1.5 rounded-lg hover:bg-white/5">
          <span>🧠</span>
          <span>我的画像</span>
        </NuxtLink>
      </div>
      ...
      <p class="text-base text-slate-400 mb-8">输入一个选择题，AI 为你推导每条路的连锁结果</p>
      ...
      <form class="max-w-2xl mx-auto" @submit.prevent="goTree">
        <div class="relative group">
          ...
          <input
            ...
            class="w-full pl-14 pr-28 py-4 rounded-2xl bg-white/5 border border-white/15 text-white text-base placeholder-slate-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
          />
          <button
            type="submit"
            :disabled="!query.trim()"
      ...
      <div class="flex flex-wrap justify-center gap-2 mt-4">
        <button
          ...
          :key="example"
          class="text-xs px-3 py-1.5 rounded-full border border-white/10 text-slate-400 hover:text-white hover:border-white/25 hover:bg-white/5 transition-all"
    ...
    <div class="flex items-center gap-4 my-8 max-w-2xl mx-auto">
      ...
      <span class="text-xs text-slate-500 uppercase tracking-wider">或选择预设场景</span>
      <div class="flex-1 h-[1px] bg-white/10" />
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-w-3xl mx-auto">
      <NuxtLink
        ...
        :key="s.id"
        :to="`/sim/${s.id}`"
        class="group relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.03] p-4 transition-all hover:border-white/20 hover:bg-white/[0.07] hover:scale-[1.02]"
      >
      ...
      <NuxtLink
        to="/custom"
        class="group relative overflow-hidden rounded-xl border border-dashed border-white/15 bg-white/[0.01] p-4 transition-all hover:border-white/30 hover:bg-white/[0.05] hover:scale-[1.02] flex flex-col items-center justify-center text-center"
      >
    ...
    <section class="mt-12 text-center text-xs text-slate-600">
